...
2. Si le contenu utile n'est pas accessible, passe par un **agrégateur public tiers** avant d'essayer une recherche générique — ces sites publient des statistiques publiques (abonnés, croissance, taux d'engagement estimé) sans mur de connexion, et sont largement plus fiables que de fouiller la plateforme d'origine à l'aveugle :
   - **Instagram / TikTok / YouTube / Twitch** : `socialblade.com/<plateforme>/user/<handle>` (ex. `socialblade.com/instagram/user/nomdecompte`) — abonnés, croissance récente, parfois taux d'engagement.
   - **Facebook** : Social Blade couvre aussi les pages Facebook dans une moindre mesure ; sinon la recherche générique (étape 3) reste le repli.
   - **LinkedIn (page d'entreprise)** : pas d'agrégateur public fiable équivalent à Social Blade — la page LinkedIn publique elle-même affiche souvent le nombre d'abonnés même sans connexion, `WebFetch` peut donc parfois le récupérer directement ; sinon repli sur l'étape 3.
   - Si un agrégateur ne retourne rien pour ce handle précis (compte trop petit pour être indexé, plateforme non couverte), continue à l'étape 3 sans t'y attarder.
3. Si ni le profil d'origine ni un agrégateur ne donnent une image fiable, essaie `WebSearch` (le nom de l'entreprise + le réseau) pour trouver des mentions, captures, ou données indexées en dernier recours.
4. Si aucune des trois ne donne une image fiable du contenu réel, **écris-le explicitement** dans le rapport ("Instagram non consultable — profil derrière un mur de connexion, non couvert par Social Blade, seule l'existence du compte a pu être confirmée") plutôt que d'estimer une fréquence de publication ou un ton que tu n'as pas réellement vus. C'est la même règle que pour les trous de données Semrush : jamais de chiffre ou d'observation inventée pour combler un accès bloqué.

Précise toujours la source dans le rapport (profil d'origine, Social Blade, ou recherche indexée) — un chiffre venant d'un agrégateur tiers n'a pas la même fiabilité qu'une donnée lue directement sur le profil officiel, et la personne qui exécute le mandat doit pouvoir juger de la marge d'erreur.
